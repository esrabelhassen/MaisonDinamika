      ...
      {orders.length === 0 ? (
        <div className="mt-8">
          ...
          <Link
            ...
            className="mt-4 inline-block rounded-full border border-glaze bg-transparent px-7 py-3 text-sm uppercase tracking-[0.08em] text-ink transition-all duration-300 hover:bg-glaze hover:text-paper hover:shadow-[0_10px_28px_-12px_rgba(114,88,34,0.5)] motion-reduce:transition-none"
          >
            {nav.voirLesProduits}
          </Link>
        </div>
            ...
            return (
              <li key={order.id}>
                <Link
                  ...
                  className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-line bg-surface/40 p-6 transition-all duration-300 hover:border-glaze hover:shadow-[0_10px_28px_-14px_rgba(114,88,34,0.4)] motion-reduce:transition-none"
                >
                  <div>
