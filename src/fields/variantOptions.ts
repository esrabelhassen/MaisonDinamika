...
// (Pack / Dimension / Couleur), each just an array of {label, image} rows.
// A section with zero rows simply doesn't render on the storefront product
// page (see VariantSelector.tsx) — there's no separate "enabled" toggle,
// leaving the array empty IS "disabled". `image` is optional per row: an
// option without one is still shown as a pickable pill, it just doesn't swap
// the gallery's active photo when clicked (see ProductDetailLayout.tsx).
